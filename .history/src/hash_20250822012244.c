...
[cite_start]// Usa o "Método da Divisão", que mapeia a chave para um índice válido na tabela. [cite: 66, 67, 69]
...
void criar_tabela_hash_funcionario(FILE *tabela_hash) {
    ...
    [cite_start]// O valor -1 indica uma "gaveta" ou compartimento vazio na tabela hash. [cite: 143]
    int pos_vazia = -1;
...
[cite_start]// Esta é a função central que implementa o encadeamento exterior. [cite: 132]
void inserir_na_lista_hash(FILE *tabela_hash, FILE *arq_funcionarios, int pos_novo_func, int cod_novo_func) {
    [cite_start]// Calcula a "gaveta" (endereço) onde o novo funcionário deve ser inserido. [cite: 175, 184]
    ...
    if (pos_cabeca_lista == -1) {
        ...
        [cite_start]// Se a gaveta não estava vazia, houve uma colisão. [cite: 101]
        [cite_start]// Precisamos percorrer a lista encadeada até o fim para adicionar o novo registro. [cite: 187]
...
// Lê um arquivo de dados de funcionários e constrói a tabela hash do zero.
// Essencial para a inicialização do programa.
void construir_hash_da_base_existente(FILE *tabela_hash, FILE *arq_funcionarios) {
    ...
    // Começa criando uma tabela hash limpa e vazia.
    criar_tabela_hash_funcionario(tabela_hash);
    // Garante que a leitura do arquivo de dados começará do início.
    rewind(arq_funcionarios);

    TFunc *func_lido;
    int pos_atual = 0; // Contador de posição do registro (0, 1, 2...).

    // Lê o arquivo de dados, um funcionário por vez, até o final.
    while ((func_lido = le_funcionario(arq_funcionarios)) != NULL) {
        // A função 'inserir_na_lista_hash' move o cursor do arquivo de funcionários.
        // É crucial salvar a posição do cursor antes da chamada e restaurá-la depois,
        // para que o loop 'while' continue a leitura do ponto correto.
        long int pos_leitura_continua = ftell(arq_funcionarios);

        // Apenas insere na hash se o funcionário estiver marcado como ativo ('ocupado').
        if (func_lido->ocupado == 1) {
            inserir_na_lista_hash(tabela_hash, arq_funcionarios, pos_atual, func_lido->cod);
        }

        // Restaura a posição do cursor no arquivo de dados.
        fseek(arq_funcionarios, pos_leitura_continua, SEEK_SET);
        
        // Libera memória e incrementa o contador de posição.
        free(func_lido);
        pos_atual++;
    }
...
TFunc* buscar_funcionario_hash(int cod, FILE *tabela_hash, FILE *arq_funcionarios) {
    [cite_start]// Calcula a "gaveta" onde o funcionário deveria estar. [cite: 175]
    ...
    [cite_start]// Inicia a travessia na lista de colisão. [cite: 176]
    TFunc* func_atual;
    while (pos_atual != -1) { // O loop continua até o fim da lista (prox == -1).
        ...
        [cite_start]// Verifica se o funcionário lido é o que procuramos E se ele está ativo. [cite: 177]
    ...
    [cite_start]// Se o loop terminar, a lista foi percorrida e o funcionário não foi encontrado. [cite: 178]
...
void remover_funcionario_hash(int cod, FILE *tabela_hash, FILE *arq_funcionarios) {
    [cite_start]// Acha a gaveta e a cabeça da lista, como na busca. [cite: 192]
    int hash_addr = funcao_hash_funcionario(cod);
    fseek(tabela_hash, hash_addr * sizeof(int), SEEK_SET);
    int pos_atual;
    if (fread(&pos_atual, sizeof(int), 1, tabela_hash) == 0 || pos_atual == -1) {
    ...
    while (pos_atual != -1) {
        ...
        [cite_start]// Procura pelo funcionário a ser removido. [cite: 193]
        if (func_atual != NULL && func_atual->cod == cod && func_atual->ocupado == 1) {
    ...
    [cite_start]// Se o loop terminar, o funcionário não foi encontrado na lista. [cite: 195]
