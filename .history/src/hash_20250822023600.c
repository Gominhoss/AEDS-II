...
void construir_hash_da_base_existente(FILE *tabela_hash, FILE *arq_funcionarios) {
    ...
    // Calcula o número total de registros no arquivo para controlar o loop.
    // Para isso, movemos o cursor para o final do arquivo (SEEK_END)...
    fseek(arq_funcionarios, 0, SEEK_END);
    // ...pegamos a posição total em bytes com ftell() e dividimos pelo tamanho de um registro.
    ...
    // Itera por cada uma das posições de registro do arquivo, de 0 até o final.
    for (int pos_atual = 0; pos_atual < total_registros; pos_atual++) {
        
        // 1. Posiciona o cursor no início exato do registro que queremos ler.
        fseek(arq_funcionarios, pos_atual * tamanho_registro_funcionario(), SEEK_SET);

        // 2. Lê os dados completos do registro daquela posição.
        ...
        // 3. Processa o registro apenas se ele estiver marcado como ativo ('ocupado').
        ...
        // 4. Libera a memória alocada para o registro lido nesta iteração.
        free(func_lido);
    }
