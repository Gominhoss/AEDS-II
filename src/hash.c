...
// Utiliza uma abordagem de duas passagens para garantir que não haja links antigos.
void construir_hash_da_base_existente(FILE *tabela_hash, FILE *arq_funcionarios) {
    ...
    // Calcula o número total de registos no ficheiro para controlar os loops.
    fseek(arq_funcionarios, 0, SEEK_END);
    int total_registros = ftell(arq_funcionarios) / tamanho_registro_funcionario();
    if (total_registros == 0) {
        printf("Base de dados vazia. Construção da hash concluída.\n");
        return;
    }

    TFunc *func_temp;

    // --- PRIMEIRA PASSAGEM: LIMPAR LINKS ANTIGOS ---
    // Percorre o ficheiro de dados para redefinir todos os ponteiros 'prox' para -1.
    // Isto é crucial para que a reconstrução da hash não seja afetada por dados de execuções anteriores.
    for (int i = 0; i < total_registros; i++) {
        fseek(arq_funcionarios, i * tamanho_registro_funcionario(), SEEK_SET);
        func_temp = le_funcionario(arq_funcionarios);
        if (func_temp != NULL) {
            // Apenas reescreve o registo se o 'prox' não for já -1, para otimizar.
            if (func_temp->prox != -1) { 
                func_temp->prox = -1;
                fseek(arq_funcionarios, i * tamanho_registro_funcionario(), SEEK_SET);
                salva_funcionario(func_temp, arq_funcionarios);
            }
            free(func_temp);
        }
    }

    // --- SEGUNDA PASSAGEM: CONSTRUIR A NOVA HASH ---
    // Agora, com os links limpos, itera novamente para inserir cada registo na estrutura da hash.
    for (int pos_atual = 0; pos_atual < total_registros; pos_atual++) {
        // Posiciona o cursor no início do registo que queremos ler.
        fseek(arq_funcionarios, pos_atual * tamanho_registro_funcionario(), SEEK_SET);
        func_temp = le_funcionario(arq_funcionarios);
        if (func_temp == NULL) { 
            break; 
        ...
        // Processa o registo apenas se ele estiver marcado como ativo ('ocupado').
        if (func_temp->ocupado == 1) {
            inserir_na_lista_hash(tabela_hash, arq_funcionarios, pos_atual, func_temp->cod);
        }
